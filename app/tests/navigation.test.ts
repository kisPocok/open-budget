...
class Ui {
    createMenu(label: string): Ui {
        return this
    }
    addItem(label: string, callback: string): Ui {
        return this
    }
    addSeparator(): Ui {
        return this
...
const dummyConfig = (enabledAutoConverter: boolean) => {
    return {
        ...
        availableConverters: [{ name: 'OTP', filename: 'converter-otp2020', class: 'OTP2020' }],
    }
}
...
test('Menu creation', () => {
    ...
    expect(mockUi.addItem).toHaveBeenNthCalledWith(
        2,
        'Import from OTP',
        'import_' + enabledAutoConverter.availableConverters[0].class
    )
...
test('Show converter button on automation disabled', () => {
    ...
    expect(mockUi.addItem).toHaveBeenNthCalledWith(
        3,
        'Import from OTP',
        'import_' + disabledAutoConverter.availableConverters[0].class
    )
    expect(mockUi.addSeparator).toHaveBeenCalled()
    expect(mockUi.addToUi).toHaveBeenCalled()
})
